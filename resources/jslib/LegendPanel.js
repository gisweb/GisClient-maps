...
OpenLayers.Control.LegendPanel = OpenLayers.Class(OpenLayers.Control, {
  ...
  // DOM Elements
    ...
    getLegendUrl: function(layer) {
        ...
        var idx = OpenLayers.Util.indexOf(layerNames, layer.name);
	...
	checkState: function(e){
		//console.log(e.layer.name + ' ' + e.layer.inRange)
		//$("[name='"+e.layer.name+"']").prop('disabled',!e.layer.inRange)
		
		//console.log(e.layer)
		
		//var node = jQuery(this.dataLayersDiv).tree('find',e.layer.gc_id);
		//jQuery(this.dataLayersDiv).tree('select', node.target);
	},
    ...
    redraw: function() {
		...
		//$('.easyui-tree').tree({data: GisClient.nodes,checkbox:true,animate:true});
...
console.log(layer)
				if(layer instanceof OpenLayers.Layer.WMS && layer.visibility && layer.inRange){

					console.log (this.getLegendUrl(layer))
					//layerNames += "," + [layer.params.LAYERS].join(",");
					var imgElem = document.createElement("img");
					imgElem.src=this.getLegendUrl(layer);
	...
	redrawnoooooooooo: function() {
	 
	 
		
		for(var i=0, len=this.map.mapConfig.nodes.length; i<len; i++) {
